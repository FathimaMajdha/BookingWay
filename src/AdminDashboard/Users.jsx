...
const Users = () => {
  ...
  const fetchUsers = async (page = 1, searchTerm = "", sortColumn = "Created_At", sortDirection = "DESC") => {
    ...
    try {
      ...
      const res = await axiosInstance.get("/admin/UserManagement", { params });
  ...
  const toggleButton = async (userId, currentStatus) => {
    ...
    try {
      setUsers((prev) => prev.map((user) => (user.UserAuthId === userId ? { ...user, IsBlocked: !currentStatus } : user)));
  ...
  const deleteButton = async (userId) => {
    const userName = users.find((u) => u.UserAuthId === userId)?.Username || "this user";
  ...
  const proceedWithDelete = async (userId) => {
    setActionLoading(userId);
    try {
      const response = await axiosInstance.delete(`/admin/UserManagement/${userId}`);
    ...
    <div className="d-flex">
      ...
      <div className="container mt-4" style={{ marginLeft: "200px" }}>
        ...
        <div className="row mb-4">
          ...
          <div className="col-md-3">
            <div className="text-muted small">Total: {totalCount} users</div>
          </div>
        </div>
          ...
          <table className="table table-striped table-hover align-middle">
            <thead className="table-dark text-center">
              <tr>
                ...
                <th style={{ cursor: "pointer" }} onClick={() => handleSort("TotalBookings")}>
              ...
              <div className="modal-content">
                ...
                <div className="modal-body">
                  <div className="row mb-4">
                    <div className="col-md-6">
                      <p>
                        <strong>Username:</strong> {selectedUser.Username || selectedUser.name}
                      </p>
                      <p>
                        <strong>Email:</strong> {selectedUser.Email || selectedUser.email}
                      </p>
                    </div>
                    <div className="col-md-6">
                      <p>
                        ...
                        <span
                          className={`badge ${
                            selectedUser.IsBlocked || selectedUser.isBlocked ? "bg-danger" : "bg-success"
                          } ms-2`}
                        >
                      ...
                      <p>
                        ...
                        {formatDate(selectedUser.RegistrationDate || selectedUser.registrationDate)}
                      </p>
                      <p>
                  ...
                  {(selectedUser.Bookings || selectedUser.bookings || []).length > 0 ? (
                    <div className="booking-list">
                      {(selectedUser.Bookings || selectedUser.bookings || []).map((order, index) => (
                        <div key={order.BookingId || order.bookingId || index} className="card mb-3">
                          <div className="card-header d-flex justify-content-between align-items-center">
                            <span
                              className={`badge ${
                                order.BookingType || order.type === "Flight" ? "bg-primary" : "bg-success"
                              }`}
                            >
                          ...
                          <div className="card-body">
                            <div className="row">
                              <div className="col-md-8">
                                ...
                                <p className="card-text mb-1">
                                  <strong>Contact:</strong> {order.CustomerEmail || order.customerEmail} |{" "}
